...
def evaluate_performance(args):
    ...
    data = np.load(f'/mmfs1/home/mdtahmid.islam/deepfri_bert/evaluation/eval_results/last_tryec_cmap_bias_per_head_alpha_no_cnn_model_preds_raw.npz')
    all_remapped_preds = []
    all_mf_labels = []
    common_keys = []
    filtered_pdb = None
    if args.percentage == "30":
        filtered_pdb = get_filtered_pdb(args.file_path_suffix)
    ...
    for chain_id in data.files:
        ...
        binary_preds[missing_indices] = 0



        # Get ground-truth MF label
        mf_label = terms[label_name]
       

        all_remapped_preds.append(binary_preds)
    ...
    print(f"{label_name}: Fmax={fmax:.3f}, MACRO_AUPR={macro_aupr:.3f},  MICRO_AUPR={micro_aupr:.3f}")

def get_filtered_pdb(file_path):
...
if __name__ == "__main__":
    ...
    parser.add_argument( '--label_name', type=str, default='mf', choices=['mf', 'bp', 'cc', 'ec'], help="Ontology.")
    parser.add_argument( '--percentage', type=str, default='100', choices=['30', '100'], help="Identity Similarity.")
